...
Состояние на 27.08.2026. Отсортировано по пользе для сдачи, а не по красоте.
...
3. **Десять тем каталога не на чем тренировать.** Konsolenbefehle, Wasserfall
   и Scrum, файловые системы, Anonymisierung/Pseudonymisierung, Projektstruktur-
   plan, Domäne, Testprotokoll, Risikomatrix, Tuckman, Betroffenenrechte DSGVO,
   Barrierefreiheit. В сборке заданий по ним нет.
...
1. **Ранг 1** — десять непокрытых тем каталога
2. **Ранг 2** — повторение по интервалам
3. **Ранг 3** — ER/UML/Gantt как таблицы
4. остальное по настроению
